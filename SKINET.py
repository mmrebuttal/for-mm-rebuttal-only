...
class SKINET(nn.Module):
    def __init__(self, in_channels, out_channels, nf, gc=32, kernel_size=3, stride=1, dilation=1, groups=1, bias=True,
        ...
        self.dense_residual_block = nn.Sequential(*basic_block_layer)
    ...
    def forward(self, x):
        ...
        x1 = self.conv1(x)
        x2 = self.dense_residual_block(x1)
        x3 = self.conv2(x2)
        x4 = self.dense_residual_block(x3)
        x5 = self.conv3(x4)
        x6 = self.dense_residual_block(x5)
        x_branch1 = self.conv3(x1+x6)
